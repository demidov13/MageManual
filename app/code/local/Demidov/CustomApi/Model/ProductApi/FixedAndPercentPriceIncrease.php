...
class Demidov_CustomApi_Model_ProductApi_FixedAndPercentPriceIncrease implements Demidov_CustomApi_Model_ProductApi_BaseInterface
{
    protected $params;
    protected $result = [];
    ...
    public function process()
    {
       ...
       if (!$collectionSimple->count()) {
        ...
        if ($this->params['type_increase'] == 'fixed') {
            ...
            foreach ($collectionSimple as $product) {
                $price = $product->getData('price');
                $price =  $price + $price * $this->params['amount'] / 100;
                $product->setData('price', $price);
                $info['id'] = $product->getData('entity_id');
                $info['sku'] = $product->getData('sku');
                $info['name'] = $product->getData('name');
